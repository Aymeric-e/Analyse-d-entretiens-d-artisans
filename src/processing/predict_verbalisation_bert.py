...
class BertPredictor:
    ...
    def save_predictions(self, filenames: pd.Series, texts: pd.Series, 
                        predictions: np.ndarray, output_path: Path) -> None:
        """Save predictions to CSV"""
        print(f"Sauvegarde des prédictions...")
        
        results_df = pd.DataFrame({
            'filename': filenames.values,
            'text': texts.values,
            'note_bert': predictions
        })
...
def main():
    parser = argparse.ArgumentParser(
        description="Générer des prédictions avec le modèle BERT"
    )
    parser.add_argument(
        "--input",
        type=Path,
        required=True,
        help="Chemin du CSV avec textes à prédire"
    )
    parser.add_argument(
        "--model-dir",
        type=Path,
